...
public struct Mesh
{
	...
	public void decode(GLib.HashTable<string, Value?> mesh)
	{
		if (mesh.contains("source")) {
			...
			string source_path_lower = source_path.down();
			if (source_path_lower.has_suffix(".gltf") || source_path_lower.has_suffix(".glb")) {
				...
				cgltf.Scene* scene = data.scene != null
					? data.scene
			...
			if (source_path_lower.has_suffix(".obj"))
