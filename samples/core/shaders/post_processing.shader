...
bgfx_shaders = {
	bloom_params = {
		...
		code = """
			...
			#define bloom_scale u_bloom_params.x
			#define bloom_inv_scale u_bloom_params.x
	...
	bloom_combine = {
		...
		fs_code = """
			...
			void main()
			{
				...
				vec3 bloom = texture2D(s_bloom_map, v_texcoord0).rgb * bloom_inv_scale;
				gl_FragColor = vec4(clamp(mix(color, bloom, bloom_weight), -65504.0, 65504.0), 1.0);
			}
		"""
	}
