...
SIMD_FORCE_INLINE const char* btGeneric6DofSpring2Constraint::serialize(void* dataBuffer, btSerializer* serializer) const
{
	...
	for (i = 0; i < 3; i++)
	{
		dof->m_angularLowerLimit.toPtr()[i] = m_angularLimits[i].m_loLimit;
		dof->m_angularUpperLimit.toPtr()[i] = m_angularLimits[i].m_hiLimit;
		dof->m_angularBounce.toPtr()[i] = m_angularLimits[i].m_bounce;
		dof->m_angularStopERP.toPtr()[i] = m_angularLimits[i].m_stopERP;
		dof->m_angularStopCFM.toPtr()[i] = m_angularLimits[i].m_stopCFM;
		dof->m_angularMotorERP.toPtr()[i] = m_angularLimits[i].m_motorERP;
		dof->m_angularMotorCFM.toPtr()[i] = m_angularLimits[i].m_motorCFM;
		dof->m_angularTargetVelocity.toPtr()[i] = m_angularLimits[i].m_targetVelocity;
		dof->m_angularMaxMotorForce.toPtr()[i] = m_angularLimits[i].m_maxMotorForce;
		dof->m_angularServoTarget.toPtr()[i] = m_angularLimits[i].m_servoTarget;
		dof->m_angularSpringStiffness.toPtr()[i] = m_angularLimits[i].m_springStiffness;
		dof->m_angularSpringDamping.toPtr()[i] = m_angularLimits[i].m_springDamping;
		dof->m_angularEquilibriumPoint.toPtr()[i] = m_angularLimits[i].m_equilibriumPoint;
	}
	dof->m_angularLowerLimit.w = 0;
	dof->m_angularUpperLimit.w = 0;
	dof->m_angularBounce.w = 0;
	dof->m_angularStopERP.w = 0;
	dof->m_angularStopCFM.w = 0;
	dof->m_angularMotorERP.w = 0;
	dof->m_angularMotorCFM.w = 0;
	dof->m_angularTargetVelocity.w = 0;
	dof->m_angularMaxMotorForce.w = 0;
	dof->m_angularServoTarget.w = 0;
	dof->m_angularSpringStiffness.w = 0;
	dof->m_angularSpringDamping.w = 0;
	dof->m_angularEquilibriumPoint.w = 0;
	for (i = 0; i < 4; i++)
	{
