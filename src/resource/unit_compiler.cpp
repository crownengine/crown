...
static s32 compile_light(Buffer &output, UnitCompiler &compiler, FlatJsonObject &obj, CompileOptions &opts)
{
	...
	if (cookie_json != NULL && sjson::type(cookie_json) != JsonValueType::NIL) {
...
namespace unit_compiler
{
	...
	s32 parse_component_override_guid(Guid &component_id, const StringView &key, CompileOptions &opts)
	{
		RETURN_IF_FALSE(UNIT_COMPILER
			, key.length() == GUID_BUF_LEN && key.data()[0] == '#'
			, opts
			, "Invalid component override ID: '%.*s'"
			, (int)key.length()
			, key.data()
			);
		...
		RETURN_IF_FALSE(UNIT_COMPILER
			, guid::try_parse(component_id, guid)
			, opts
			, "Invalid component override ID: '%.*s'"
			, (int)key.length()
			, key.data()
